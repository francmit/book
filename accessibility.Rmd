...
would define a unsemantic `div` element that is _styled_ (via a developer-defined CSS class) to look like a button. But in order to tell the screen reader that this element can be clicked like a button, it is given the `role` of `"button"`. There are a [large number](https://www.w3.org/TR/wai-aria/roles#roles_categorization_header) of different roles (following a potentially complex classification scheme), and include [roles for interactive widgets](https://www.w3.org/TR/wai-aria/roles#widget_roles) such as a popup `dialog`, a `progressbar`, or a `menu`. These are important when making complex interactions, though for most simpler pages you can stick with the standard semantic elements (e.g., a `<button>` instead of a custom `<div>`).
